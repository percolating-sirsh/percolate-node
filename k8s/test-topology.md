...
**Issue**: KEDA operator crashes with "failed to wait for scaledjob caches to sync"

**Root cause**: ScaledJob CRD missing from initial KEDA installation

**Fix**: Reinstall KEDA with Helm to ensure all CRDs are installed:

```bash
# Delete existing KEDA resources
kubectl delete namespace keda
kubectl delete crd $(kubectl get crd | grep keda | awk '{print $1}')
kubectl delete clusterrole $(kubectl get clusterrole | grep keda | awk '{print $1}')
kubectl delete clusterrolebinding $(kubectl get clusterrolebinding | grep keda | awk '{print $1}')
kubectl delete apiservice v1beta1.external.metrics.k8s.io

# Install with Helm
helm repo add kedacore https://kedacore.github.io/charts
helm repo update
helm install keda kedacore/keda --namespace keda --create-namespace --version 2.13.0 --wait
```

**Note**: Even with operator crashes, existing ScaledObjects continue to work through their created HPAs
...
- [system.md](system.md) - Database-first architecture design
- [kind.md](kind.md) - Full stack local testing
- [OBSERVABILITY.md](test-topology/OBSERVABILITY.md) - Complete observability architecture
