...
pub fn robust_prune(
    ...
    for (candidate_id, candidate_dist) in sorted_candidates {
        // Always add first candidate (closest)
        if selected.is_empty() {
            selected.push(candidate_id);
            continue;
        }

        // Check if candidate is diverse enough from already selected neighbors
        if is_diverse(candidate_id, candidate_dist, &selected, vectors, alpha) {
            selected.push(candidate_id);

            // Stop once we have enough neighbors
...
fn is_diverse(
    ...
    // Check if candidate is diverse from all selected neighbors
    for &selected_id in selected {
        let selected_vec = &vectors[selected_id as usize];
        let inter_distance = euclidean_distance(candidate_vec, selected_vec);

        // Diversity criterion: candidate_dist < alpha * distance(candidate, selected)
        // If candidate is too close to an existing neighbor, reject it
