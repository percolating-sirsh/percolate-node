...
use crate::index::diskann::graph::GraphNeighbors;
...
/// ```
pub fn greedy_search<G: GraphNeighbors>(
    graph: &G,
    vectors: &[Vec<f32>],
    query: &[f32],
    medoid: u32,
    ...
    // Validate medoid
    if (medoid as usize) >= graph.num_nodes() {
    ...
    let mut candidates = BoundedPriorityQueue::new(params.search_list_size);
    let mut best = BoundedPriorityQueue::new(params.top_k);

    // Start from medoid
    let medoid_dist = compute_distance(query, &vectors[medoid as usize]);
    ...
    while let Some(current) = candidates.pop() {
        ...
        // Explore neighbors
        let neighbors = graph.neighbors(current.node_id);
        for &neighbor in neighbors {
            if !visited.contains(&neighbor) {
                let dist = compute_distance(query, &vectors[neighbor as usize]);
                candidates.push(SearchCandidate {
                    node_id: neighbor,
                    distance: dist,
                });
...
impl BoundedPriorityQueue {
    ...
    fn push(&mut self, candidate: SearchCandidate) -> bool {
        if self.heap.len() < self.max_size {
            ...
            // Check if this candidate is better than the worst one
